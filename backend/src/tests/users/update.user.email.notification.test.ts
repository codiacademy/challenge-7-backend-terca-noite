...
import * as notifications from "../../functions/notifications/send-overview-email-to-user-function.ts";
...
// MOCK: Substitui a função real por um mock (vi.spyOn)
const sendOverviewEmailMock = vi
  .spyOn(notifications, "sendOverviewEmailtoUser")
  .mockImplementation(async (userId: string) => {
    console.log(`[MOCK] E-mail de Visão Geral SIMULADO enviado para o User ID: ${userId}`);
    return Promise.resolve(); // Simula o sucesso
  });

// Dados do usuário mockado
const MOCKED_USER = {
  fullName: "Email Notification Test User",
  email: "email.notify.test@codicash.com",
...
beforeAll(async () => {
  ...
  // Assumimos que 'createTestUser' permite definir o estado inicial
...
afterAll(async () => {
  ...
  sendOverviewEmailMock.mockRestore(); // Restaura a função original
  await appInstance.close();
});

// --- Testes da Rota /update_email_notification ---

describe("PATCH /update_email_notification - Atualização de Notificações por Email", () => {
  it("1. Deve mudar o status de FALSE para TRUE e chamar a função de envio de email", async () => {
    // 1. Garante que o estado inicial é FALSE
    let dbUser = await prisma.user.findUnique({
      where: { id: testUserId },
      select: { notification_email_enabled: true },
    });
    expect(dbUser?.notification_email_enabled).toBe(false); // 2. Executa a requisição PATCH (deve ir para TRUE)

    const response = await requestClient.patch("/users/update_email_notification").set({
    ...
    expect(dbUser?.notification_email_enabled).toBe(true); // 5. Verificação do MOCK (Garante que o email foi tentado)

    expect(sendOverviewEmailMock).toHaveBeenCalledTimes(1);
    expect(sendOverviewEmailMock).toHaveBeenCalledWith(testUserId);
  });
  ...
  it("2. Deve mudar o status de TRUE de volta para FALSE na segunda chamada", async () => {
    ...
    expect(dbUser?.notification_email_enabled).toBe(false); // 4. Verificação do MOCK: Chamado pela segunda vez

    expect(sendOverviewEmailMock).toHaveBeenCalledTimes(2);
    expect(sendOverviewEmailMock).toHaveBeenLastCalledWith(testUserId);
  });
  ...
  it("3. Deve retornar 401 Unauthorized se o token de autenticação estiver ausente", async () => {
    ...
    const response = await requestClient.patch("/users/update_email_notification"); // O preHandler [app.authenticate] deve bloquear
